...
//   GET  /api/checkout/link     // redirect straight to Stripe
...
// Pin API version so deploys are stable
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY, { apiVersion: '2025-07-30.basil' });

// Use JSON for normal routes; raw body only for the Stripe webhook
app.use((req, res, next) => {
  if (req.originalUrl.startsWith('/api/stripe/webhook')) next();
  else express.json({ limit: '1mb' })(req, res, next);
});

// In production, restrict to your Softr/site origins
app.use(
  cors({
    origin: ['https://www.imladrisarchives.com', 'https://imladrisarchives.softr.app'],
  })
);
...
async function fetchProductsByIds(ids) {
  ...
  const records = await base(T.products)
    .select({ filterByFormula: formula, maxRecords: ids.length })
    .all();
...
async function getShippingRate(tier, country) {
  ...
  const [rec] = await base(T.shipping)
...
function consolidateShippingTiers(items) {
  ...
  for (let i = tubeOrder.length - 1; i >= 0; i--) {
    if (tubes.includes(tubeOrder[i])) {
      largestTube = tubeOrder[i];
      break;
    }
  }
  const needed = [];
...
async function placeHolds(items) {
  ...
  const updates = items.map((i) => ({
    id: i.id,
    fields: { Status: 'On Hold', HoldUntil: holdUntil },
  }));
...
async function releaseHoldsByIds(ids) {
  ...
  const updates = ids.map((id) => ({
    id,
    fields: { Status: 'Available', HoldUntil: null },
  }));
  for (let i = 0; i < updates.length; i += 10) {
    await base(T.products).update(updates.slice(i, i + 10));
  }
...
async function markSold(items, orderId) {
  const updates = items.map((i) => ({
    id: i.id,
    fields: { Status: 'Sold', Quantity: 0, SoldInOrder: [orderId] },
  }));
  for (let i = 0; i < updates.length; i += 10) {
    await base(T.products).update(updates.slice(i, i + 10));
  }
...
async function createCheckoutFromCart(cart, country = 'US', customer_email) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    shipping_address_collection: {
      allowed_countries: ['US', 'CA', 'GB', 'IE', 'FR', 'DE', 'ES', 'IT', 'AU', 'NZ'],
    },
    shipping_options,
    ...
    metadata: { airtable_ids: items.map((i) => i.id).join(','), hold_until: holdUntil },
  });
...
app.post('/api/cart/price', async (req, res) => {
  try {
    ...
    if (!Array.isArray(cart) || cart.length === 0)
      return res.json({ items: [], subtotal: 0, shipping: 0, total: 0, currency: 'usd' });
    ...
    for (const tier of neededTiers) {
      ...
      if (rate) {
        ...
        shippingBreakdown.push({ tier, ...rate });
      }
    }
...
// POST checkout (used by Softr “Call API”)
...
// GET checkout link (perfect for Softr “Open URL”)
app.get('/api/checkout/link', async (req, res) => {
  try {
    ...
    if (out.error === 'out_of_stock') return res.redirect(`${process.env.SITE_URL}/sold-out`);
...
// Optional convenience POST for “Buy Now”
app.post('/api/buy-now', async (req, res) => {
  try {
    ...
    const out = await createCheckoutFromCart(
      [{ id: productId, qty: Number(qty) || 1 }],
      country,
      customer_email
    );
    if (out.error) return res.status(409).json(out);
...
app.post('/api/stripe/webhook', bodyParser.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      // Create Order row (fields constrained to ones that exist)
      ...
      // Link items and mark Sold
      const productRecords = await fetchProductsByIds(ids);
      const orderItemCreates = productRecords.map((p) => ({
        fields: { Order: [orderId], Product: [p.id], Name: p.Name, SKU: p.SKU, Price: p.Price, Qty: 1 },
      }));
